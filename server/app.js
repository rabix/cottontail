...
try {
    ...
    if (exception.name === 'SyntaxError') {
        ...
        var versionIsSatisfied = true;
        var versionRequirement = [4, 2, 0];
        var processVersion = process.version.slice(1).split('.').map(function (num) {
            return ~~num
        });


        for (var i = 0; i < 3; i++) {
            if (processVersion[i] > versionRequirement[i]) {
                versionIsSatisfied = true;
                break;
            } else if (processVersion[i] < versionRequirement[i]) {
        ...
        if (!versionIsSatisfied) {
            console.warn('Hey there.\nWe have encountered an error that\'s most likely ' +
                ...
                '\n\nP.S. Here\'s the error: ' + exception
                ,
                process.version,
